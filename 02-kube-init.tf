resource "null_resource" "init_main_master" {
  ...
  provisioner "file" {
    source      = "${path.module}/scripts/kube-bootstrap.sh"
  ...
  provisioner "file" {
    source      = "${path.module}/scripts/kube-main-master.sh"
...
resource "null_resource" "init_masters" {
  ...
  provisioner "file" {
    source      = "${path.module}/scripts/kube-master.sh"
...
resource "null_resource" "init_workers" {
  ...
  provisioner "local-exec" {
    command = "bash ${path.module}/scripts/copy-join-token.sh"
  ...
  provisioner "file" {
    source      = "${path.module}/scripts/kube-node.sh"
